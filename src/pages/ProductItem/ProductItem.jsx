...
const ProductItem = () => {
  ...
  const colors = [
    { label: "Graphite", value: "#3C4242" },
    { label: "Sunshine", value: "#EDD146" },
    { label: "Blossom", value: "#EB84B0" },
    { label: "Ruby", value: "#9C1F35" },
  ];
  const price = product.height ? (product.height / 100).toFixed(2) : null;
  ...
  const isManProduct = product.alt?.toLowerCase().includes("man");
  const productCategory = isManProduct ? "Men's Essentials" : "Women's Essentials";
  const productTitle = product.alt || "Premium Product";
  ...
  return (
    <div className="product-item-page">
        ...
        <section className="product-grid">
          <article className="product-card">
            <img
              ...
              src={product.src.large2x}
              alt={product.alt || "Imagem do produto"}
            />
            <div className="product-card-footer">
              <div className="product-badges">
                <span className="product-badge">Featured</span>
                <span className="product-badge">Premium pick</span>
              </div>
              <p className="product-photographer">
                Photo by <strong>{product.photographer}</strong>
              </p>
            </div>
          </article>
          ...
          <article className="product-content">
            <div className="product-header">
              <div>
                <p className="product-subtitle">{productCategory}</p>
                <h1>{productTitle}</h1>
              </div>
              <div className="product-panel-price">${price}</div>
            </div>
            ...
            <div className="product-rating">
              ...
              <span>4.5 • 184 reviews</span>
            </div>

            <p className="product-description">
              A polished product showcase with refined details and a more modern
              shopping experience. Designed for clean navigation and fast
              selection.
            </p>
            ...
            <div className="product-actions">
              ...
              <div className="option-group">
                <p className="option-label">Colors available</p>
      ...
      <div>
        {product.id && product.alt?.includes("Man") ? <CardsMens /> : <CardsWoman />}
        <Footer />
      </div>
    </div>
  );
